...
############################################## Random Forest ########################################
		...
		tobj.rf <- tune.randomForest(Aggregate ~ .,data= train[,2:length(train)], ntree = try )
		bestNtree <- tobj.rf$best.parameters[[1]]	
		Data_Set.rf = randomForest(train$Aggregate ~ ., data = train[,2:length(train)], importance=TRUE, ntree = bestNtree, mtry=sqrt(length(train)-1), replace=TRUE, localImp=TRUE)
		pred.rf = predict(Data_Set.rf, newdata = test[,2:(length(test)-1)])
		mc.rf = table(test$Aggregate,pred.rf)
		all.mc.rf = all.mc.rf + mc.rf
		
		pred.rf = predict(Data_Set.rf,newdata = test[,2:(length(test)-1)], type="prob")
		RF_predictions = pred.rf[,2]
		RF_labels = test$Aggregate

	############################################# Decision TREE ###############################	
		Data_Set.tree = rpart(train$Aggregate ~ ., data=train[,2:length(train)],  method="class", control= rpart.control(minsplit = 20, minbucket = round(20/3), cp = 0.01, maxcompete = 3, maxsurrogate = 4, usesurrogate = 1, xval = 0,surrogatestyle = 0, maxdepth = 30))
		pred.tree = predict(Data_Set.tree,newdata=test[,2:(length(test)-1)], type="class")
		mc.tree = table(test$Aggregate, pred.tree)
		all.mc.tree = all.mc.tree + mc.tree
		
		pred.tree = predict(Data_Set.tree,newdata=test[,2:(length(test)-1)], type="prob")
		TREE_predictions = pred.tree[,2]
		TREE_labels = test$Aggregate
		
	############################################# Support Vector Machine ###############################	
		test_seq = 2^(-3:10)	
		tobj.svm = tune.svm(Aggregate ~ .,data= train[,2:length(train)], gamma = test_seq, cost = test_seq)
		bestGamma = tobj.svm$best.parameters[[1]]
		bestC = tobj.svm$best.parameters[[2]]	
		Data_Set.svm = svm(train$Aggregate ~ ., train[,2:length(train)], type='C', kernel="radial", cost = bestC, gamma = bestGamma, probability = TRUE)
		pred.svm = predict(Data_Set.svm, newdata = test[,2:(length(test)-1)],probability = TRUE,decision.values = TRUE)
	...
	############################################ Logistic Regression ################################   
		Data_Set.brglm = brglm(train$Aggregate ~ .,family=binomial(logit),data=train[,2:length(Data_Set)],method = "brglm.fit")
		pred.brglm = predict(Data_Set.brglm,newdata=test[,2:(length(test)-1)])
	...
	############################################ Linear Discremenant Analysis ###########################
		Data_Set.lda = lda(train$Aggregate ~ .,train[,2:length(train)])
		pred.lda = predict(Data_Set.lda,newdata=test[,2:(length(test)-1)], type="class")
	...
	######################################## k-Nearest Neighbor ########################################
		tobj.kknn = train.kknn(train$Aggregate ~ ., train[,2:length(train)], kmax = 15,kernel = c("triangular", "rectangular", "epanechnikov", "biweight", "triweight", "cos", "inv", "gaussian"), distance = 1)
		bestKernel = tobj.kknn$best.parameters[[1]]
		bestK = tobj.kknn$best.parameters[[2]]
		Data_Set.kknn = kknn(train$Aggregate ~ ., train[,2:length(train)], test[,2:(length(test)-1)], k = bestK, distance = 1,kernel = bestKernel)
		pred.kknn = fitted(Data_Set.kknn)
	...
	######################################### NN ###################################################
		tobj.nn = tune.nnet(Aggregate ~ ., data = train[,2:length(train)], size = c(1,5,10,15),decay = c(0,0.001,0.1))
		bestSize = tobj.nn$best.parameters[[1]]
		bestDecay = tobj.nn$best.parameters[[2]]
		Data_Set.nn = nnet(train$Aggregate ~ ., data = train[,2:length(train)], size = bestSize, rang = 0.1,decay = bestDecay, maxit = 200)
		pred.nn = predict(Data_Set.nn,newdata = test[,2:(length(test)-1)], type="class")
		mc.nn = table(test$Aggregate,pred.nn)
		all.mc.nn = all.mc.nn + mc.nn
		
		pred.nn = predict(Data_Set.nn,newdata = test[,2:(length(test)-1)], type="raw")
		NN_predictions = pred.nn[,1]
		NN_labels = test$Aggregate
		
		##################################### naiveBayes  #########################################
		Data_Set.nb = naiveBayes(train$Aggregate ~ ., data = train[,2:length(train)])
		pred.nb = predict(Data_Set.nb, newdata = test[,2:(length(test)-1)], type="class")
		mc.nb = table(test$Aggregate, pred.nb)
		all.mc.nb = all.mc.nb + mc.nb
		
		pred.nb = predict(Data_Set.nb, newdata = test[,2:(length(test)-1)], type="raw")
...
for(r in 1:8)
{
	for(c in 1:8)
	{
		mean_arr[r,c] = mean(arr[r,c,1:7])
		STD_arr[r,c] = sd(arr[r,c,1:7])
	}
}
...
for(i in 1:8)
{
	...
	if(i ==1)
	{
	plot( get(paste("perf.PR.",Names[i], sep="")) ,lwd=3,avg="threshold", col= mycolors[i])
	title(main = paste(Names[i],"Averaged ROC curve"))
	}
	plot( get(paste("perf.PR.",Names[i], sep="")) ,lwd=3,avg="threshold",col= mycolors[i],add=TRUE)
}
...
for(i in 1:8)
{
	...
	assign(paste("perf.AUC.",Names[i], sep=""),performance(pred, "auc"))
	win.graph()
	plot( get(paste("perf.roc.",Names[i], sep="")) ,lwd=3,avg="threshold",spread.estimate="boxplot")
	title(main = paste(Names[i],"Averaged ROC curve"))
	legend("bottomright", "(x,y)", pch=1, title="Legend", legend= paste("AUC :",format(mean(as.numeric(get(paste("perf.AUC.", Names[i],sep=""))@y.values)),digits = 4) ) )
...
save(list = ls(all=TRUE), file = "10_2_without_ddG.RData")
...
print(Best_F_Measure)
 ...
 dfm=melt(x[,c(1,2,7)]) 
 win.graph()
 print(qplot(as.factor(x=X1),y=value,geom="histogram",binwidth = 10,data=dfm,fill=X2,position="dodge", xlab="Classifiers", ylab="Percentage %",histogram(binwidth=0.5)) )
