...
Names =  c("tree","svm","brglm","lda","rf","kknn","nn","nb")
	...
	if(p == 1)
	{
		for(i in 1:(get(paste(Short_Names[p],"_NO_Number",sep=""))-get(paste(Short_Names[p],"_YES_Number",sep=""))))
		{
			random_number = sample(nrow(get(paste(Short_Names[p],"_YES",sep=""))),1)
			TMP3 = get(paste(Short_Names[p],"_YES",sep=""))[random_number,]
			assign(paste(Short_Names[p],"_YES",sep=""), rbind(get(paste(Short_Names[p],"_YES",sep="")),TMP3))
			
		}
	}

}
NEW_TMP = rbind(Acp_YES,HypF_YES,P53_YES,PAH_YES,PTEN_YES,A_GalA_YES,b2m_YES,Acp_NO,HypF_NO,P53_NO,PAH_NO,PTEN_NO,A_GalA_NO,b2m_NO)
...
for(j in 1:length(Proteins))
{
	...
	Test.Set = NEW_TMP[NEW_TMP$Protein_Name == Proteins[j], ]
...
mc.adaboost = array(list(NULL), c(1,length(Proteins)))
...
train = Traing.Set
test = Test.Set
adaboost <- boosting(Aggregate ~ .,data= train[,3:length(train)], boos=TRUE,mfinal=10,control=rpart.control(cp=0.01))
adaboost.pred <- predict.boosting(adaboost,newdata= test[,3:length(test)])
mc.adaboost[[1,j]] = table(test$Aggregate,adaboost.pred$class)

}
for(i in 1:7)
{
	print(mc.adaboost[[1,i]])
}
...
#save(list = ls(all=TRUE), file = "Balanced_Data.RData")
...
table(NEW_TMP$Aggregate,NEW_TMP$Protein_Name)