...
mc.brglm = array(list(NULL), c(1,length(Proteins)))
...
for (i in 1:7)
	{
	if(length(mc.brglm[[1,i]]) == 2)
	{
		if(dimnames(mc.brglm[[1,i]])[2] == "YES")
		{
			mc.brglm[[1,i]] = cbind(matrix(0, 2, 1),mc.brglm[[1,i]])
		}
	else if (dimnames(mc.brglm[[1,i]])[2] == "NO")
		{
		 mc.brglm[[1,i]] = cbind(mc.brglm[[1,i]],matrix(0, 2, 1))
		}
...
Total_Accuracy(sum)
	...
	win.graph()
mycolors = c("red","blue","green","grey","yellow","black","cyan3","pink")

colors()
	Names =  c("TREE","SVM","BRGLM","LDA","RF","KKNN","NN","NB")
########################################################################
##################### A For loop to Precision recall curve or recall or specificity curve
for(i in 1:8)
{

	pred <- prediction(get(paste(Names[i], "_predictions2", sep="")), get(paste(Names[i], "_labels2", sep="")))
...
for (i in 1:8)
{
 x = cbind(get(paste("TP_",Names[i],"2",sep="")),get(paste("TN_",Names[i],"2",sep="")),get(paste("FP_",Names[i],"2",sep="")),get(paste("FN_",Names[i],"2",sep=""))) 
 x = matrix(x,length(Proteins),4,byrow=F) 
 rownames(x) = Short_Names
 colnames(x) = c("TP","TN","FP","FN")
 dfm=melt(x) 
 win.graph()
 print(qplot(as.factor(x=X1),y=value,geom="histogram",data=dfm,fill=X2, main=paste("Classifier",Names[i]), xlab="Proteins", ylab="counts") )
 #dev.copy(pdf,paste(classifiers_Names[i],'.pdf',sep=""))
 #dev.off()
}
