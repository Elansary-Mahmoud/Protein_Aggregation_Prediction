...
############################################## Random Forest ########################################
		...
		Data_Set.rf = randomForest(train$Aggregate ~ ., data = train[,3:length(train)], importance=TRUE, ntree = bestNtree, mtry=sqrt(length(train)-1), replace=FALSE)
	...
	######################################## k-Nearest Neighbor ########################################
		tobj.kknn = train.kknn(train$Aggregate ~ ., train[,3:length(train)], kmax = 5,kernel = c("triangular", "rectangular", "epanechnikov", "biweight", "triweight", "cos", "inv", "gaussian"), distance = 1)
...
Names =  c("tree","svm","brglm","lda","rf","kknn","nn","nb")
	...
	if(p != 6)
	{
		assign(paste(Short_Names[p],"_YES",sep=""), get(paste(Short_Names[p],"_YES",sep=""))[-c(1:(get(paste(Short_Names[p],"_YES_Number",sep=""))-get(paste(Short_Names[p],"_NO_Number",sep="")))),])
	}
...
#save(list = ls(all=TRUE), file = "undersampling_2.RData")


	
mean_arr = array(0, c(2,8))
STD_arr = array(0, c(2,8))
for(r in 1:2)
{
	for(c in 1:8)
	{
...
for(class in 1:1)
{
	...
	for(pro in 1:length(Proteins))
	{
		...
		#print(RES[class,pro])
		print(Total_Accuracy(RES[[class,pro]])[[2]])
		sen = sen + Total_Accuracy(RES[[class,pro]])[[1]]
		spec = spec + (Total_Accuracy(RES[[class,pro]]))[[2]]
	}
}
...
for(i in 1:8)
{
	print(get(paste("Total_Confusion_",Names[i],sep="")))
}

Total_Accuracy(Total_Confusion_tree)
...
#
	...
	Data_Set.tree = rpart(train$Aggregate ~ ., data=train[,3:length(train)],  method="class", control= rpart.control(minsplit = 20, minbucket = round(20/3), cp = 0.01, maxcompete = 3, maxsurrogate = 4, usesurrogate = 1, xval = 0,surrogatestyle = 10, maxdepth = 30))
	pred.tree = predict(Data_Set.tree,newdata=test[,3:(length(test)-1)], type="class")
	mc.tree[[1,j]] = table(test$Aggregate, pred.tree)
	win.graph()
	#drawTreeNodes(Data_Set.tree,print.levels = TRUE, new = TRUE) 

cex=0.7
drawTreeNodes(Data_Set.tree, cex = par("cex"), pch = par("pch"),
...
print.levels = TRUE, new = TRUE)
	...
	#text(Data_Set.tree,splits=T,all=T)              # puts names on tree plot
	#text(Data_Set.tree, use.n=TRUE, all=TRUE, cex=.5)         # puts names on tree plot
#
	#
	#try = seq(500,1000,100)
	#tobj.rf <- tune.randomForest(Aggregate ~ .,data= train[,3:length(train)], ntree = try )
	#bestNtree <- tobj.rf$best.parameters[[1]]	
	#Data_Set.rf = randomForest(train$Aggregate ~ ., data = train[,3:length(train)], importance=TRUE, ntree = bestNtree, mtry=sqrt(length(train)-1), replace=FALSE, cutoff=c(0.50,0.50))
	#pred.rf = predict(Data_Set.rf, newdata = test[,3:(length(test)-1)])
	...
	#mc.svm[[1,j]] = table(test$Aggregate, pred.svm[1:nrow(test)])
		...
		#tobj.nn = tune.nnet(Aggregate ~ ., data = train[,3:length(train)], size = 1:5,decay = c(0,0.001,0.1))
		#bestSize = tobj.nn$best.parameters[[1]]
		#bestDecay = tobj.nn$best.parameters[[2]]
		#Data_Set.nn = nnet(train$Aggregate ~ ., data = train[,3:length(train)], size = bestSize, rang = 0.1,decay = bestDecay, maxit = 200)
		#pred.nn = predict(Data_Set.nn,newdata = test[,3:(length(test)-1)], type="class")
...
for (i in 1:7)
	{
	if(length(mc.nn[[1,i]]) == 2)
	{
		if(dimnames(mc.nn[[1,i]])[2] == "YES")
		{
			mc.nn[[1,i]] = cbind(matrix(0, 2, 1),mc.nn[[1,i]])
		}
	else if (dimnames(mc.nn[[1,i]])[2] == "NO")
		{
		 mc.nn[[1,i]] = cbind(mc.nn[[1,i]],matrix(0, 2, 1))
		}
...
cor(Data_Set[,-c(1,2,18,21,22)])
 ...
 qplot(as.factor(x=X1),y=value,geom="histogram",data=dfm, fill=X2, position="dodge", xlab="Classifiers", ylab="Percentage %", cex.axis = 20) 