...
library('ggplot2')
library('reshape') 
Short_Names = c('Acp','HypF-N','P53','PAH','PTEN','SERCA2','SOD1','Spc-SH3','alpha-GalA','b2m','neuroserpin')
...
#save(list = ls(all=TRUE), file = "1ProteinOut_NewData_with_stretch.RData")

Results = array(0,c(2,8))
...
for(j in 1:length(Proteins))
{
	...
	row.names(Test.Set)=c(1:nrow(Test.Set))
	print("The Test Set")
	print(table(Test.Set$Aggregate))
