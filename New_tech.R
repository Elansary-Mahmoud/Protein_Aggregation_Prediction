...
Data_Set = Query();
Proteins = c('Acp','HypF-N','P53','PAH','PTEN','alpha-GalA','b2m')
Short_Names =  c('Acp','HypF','P53','PAH','PTEN','A_GalA','b2m')
	...
	for(p in 1:length(Proteins))
{
	...
	assign(paste(Short_Names[p],"_Training_YES_Number",sep=""),floor(get(paste(Short_Names[p],"_YES_Number",sep="")) * (2/3)))
	assign(paste(Short_Names[p],"_Training_NO_Number",sep=""),floor(get(paste(Short_Names[p],"_NO_Number",sep="")) * (2/3)))

	assign(paste(Short_Names[p],"_Train",sep=""),rbind(get(paste(Short_Names[p],"_YES",sep=""))[1:get(paste(Short_Names[p],"_Training_YES_Number",sep="")),],get(paste(Short_Names[p],"_NO",sep=""))[1:get(paste(Short_Names[p],"_Training_NO_Number",sep="")),]))
	assign(paste(Short_Names[p],"_Test",sep=""),rbind(get(paste(Short_Names[p],"_YES",sep=""))[(get(paste(Short_Names[p],"_Training_YES_Number",sep=""))+1):nrow(get(paste(Short_Names[p],"_YES",sep=""))),],get(paste(Short_Names[p],"_NO",sep=""))[(get(paste(Short_Names[p],"_Training_NO_Number",sep=""))+1):nrow(get(paste(Short_Names[p],"_NO",sep=""))),]))
...
Traing.Set = switch(j,
	rbind(HypF_Train,P53_Train,PAH_Train,PTEN_Train,A_GalA_Train,b2m_Train) 
	,rbind(Acp_Train,P53_Train,PAH_Train,PTEN_Train,A_GalA_Train,b2m_Train)
 	,rbind(Acp_Train,HypF_Train,PAH_Train,PTEN_Train,A_GalA_Train,b2m_Train)
	,rbind(Acp_Train,HypF_Train,P53_Train,PTEN_Train,A_GalA_Train,b2m_Train)
	,rbind(Acp_Train,HypF_Train,P53_Train,PAH_Train,A_GalA_Train,b2m_Train)
	,rbind(Acp_Train,HypF_Train,P53_Train,PAH_Train,PTEN_Train,b2m_Train)
	,rbind(Acp_Train,HypF_Train,P53_Train,PAH_Train,PTEN_Train,A_GalA_Train))
row.names(Traing.Set)=c(1:nrow(Traing.Set))
Traing.Set = Traing.Set[sample.int(nrow(Traing.Set)),]
row.names(Traing.Set)=c(1:nrow(Traing.Set))

Test.Set = switch(j,
	rbind(HypF_Test,P53_Test,PAH_Test,PTEN_Test,A_GalA_Test,b2m_Test) 
	,rbind(Acp_Test,P53_Test,PAH_Test,PTEN_Test,A_GalA_Test,b2m_Test)
 	,rbind(Acp_Test,HypF_Test,PAH_Test,PTEN_Test,A_GalA_Test,b2m_Test)
	,rbind(Acp_Test,HypF_Test,P53_Test,PTEN_Test,A_GalA_Test,b2m_Test)
	,rbind(Acp_Test,HypF_Test,P53_Test,PAH_Test,A_GalA_Test,b2m_Test)
	,rbind(Acp_Test,HypF_Test,P53_Test,PAH_Test,PTEN_Test,b2m_Test)
	,rbind(Acp_Test,HypF_Test,P53_Test,PAH_Test,PTEN_Test,A_GalA_Test))
...
#############################TEST SET 2#######################################
	...
	#assign(paste(Names[1], "_predictions", sep="")[[1]],r[[2]][[1]])
	TREE_predictions2[[j]] = r[[2]][[1]]
	TREE_labels2[[j]] = r[[2]][[2]]
	SVM_predictions2[[j]] = r[[2]][[3]]
	SVM_labels2[[j]] = r[[2]][[4]]
	BRGLM_predictions2[[j]] = r[[2]][[5]]
	BRGLM_labels2[[j]] = r[[2]][[6]]
	LDA_predictions2[[j]] = r[[2]][[7]]
	LDA_labels2[[j]]  = r[[2]][[8]]
	RF_predictions2[[j]] = r[[2]][[9]]
	RF_labels2[[j]] = r[[2]][[10]]
	KKNN_predictions2[[j]] = r[[2]][[11]]
	KKNN_labels2[[j]] = r[[2]][[12]]
	NN_predictions2[[j]] = r[[2]][[13]]
	NN_labels2[[j]] = r[[2]][[14]]
	NB_predictions2[[j]] = r[[2]][[15]]
	NB_labels2[[j]] = r[[2]][[16]]
...
#save(list = ls(all=TRUE), file = "NEW_TECH.RData")
...
arr2 = array(0, c(8,8,length(Proteins)))
for(j in 1:length(Proteins))
{
	...
	print (Proteins[j])
	print (Names[i])
	if(length(RES2[[i,j]]) == 2)
	{
		if(dimnames(RES2[[i,j]])[2] == "YES")
		{
			RES2[[i,j]] = cbind(matrix(0, 2, 1),RES2[[i,j]])
		}
	else if (dimnames(RES2[[i,j]])[2] == "NO")
		{
		 RES2[[i,j]] = cbind(RES2[[i,j]],matrix(0, 2, 1))
		}
...
dimnames(arr2)[[1]] = c("SEN","SPEC","ACC","ERROR","FDR","MCC","PREC","F-Measure")
dimnames(arr2)[[2]] = c("tree","svm","brglm","lda","rf","kknn","nn","nb")
dimnames(arr2)[[3]] = Proteins



TP_tree2 = array(0,c(1,length(Proteins)))
TN_tree2 = array(0,c(1,length(Proteins)))
FP_tree2 = array(0,c(1,length(Proteins)))
FN_tree2 = array(0,c(1,length(Proteins)))

TP_svm2 = array(0,c(1,length(Proteins)))
TN_svm2 = array(0,c(1,length(Proteins)))
FP_svm2 = array(0,c(1,length(Proteins)))
FN_svm2 = array(0,c(1,length(Proteins)))

TP_brglm2 = array(0,c(1,length(Proteins)))
TN_brglm2 = array(0,c(1,length(Proteins)))
FP_brglm2 = array(0,c(1,length(Proteins)))
FN_brglm2 = array(0,c(1,length(Proteins)))

TP_lda2 = array(0,c(1,length(Proteins)))
TN_lda2 = array(0,c(1,length(Proteins)))
FP_lda2 = array(0,c(1,length(Proteins)))
FN_lda2 = array(0,c(1,length(Proteins)))

TP_rf2 = array(0,c(1,length(Proteins)))
TN_rf2 = array(0,c(1,length(Proteins)))
FP_rf2 = array(0,c(1,length(Proteins)))
FN_rf2 = array(0,c(1,length(Proteins)))

TP_kknn2 = array(0,c(1,length(Proteins)))
TN_kknn2 = array(0,c(1,length(Proteins)))
FP_kknn2 = array(0,c(1,length(Proteins)))
FN_kknn2 = array(0,c(1,length(Proteins)))

TP_nn2 = array(0,c(1,length(Proteins)))
TN_nn2 = array(0,c(1,length(Proteins)))
FP_nn2 = array(0,c(1,length(Proteins)))
FN_nn2 = array(0,c(1,length(Proteins)))

TP_nb2 = array(0,c(1,length(Proteins)))
TN_nb2 = array(0,c(1,length(Proteins)))
FP_nb2 = array(0,c(1,length(Proteins)))
FN_nb2 = array(0,c(1,length(Proteins)))

for(j in 1:length(Proteins))
{
	TP_tree2[j] = RES2[[1,j]][2,2]
	TN_tree2[j] = RES2[[1,j]][1,1]
	FP_tree2[j] = RES2[[1,j]][1,2]
	FN_tree2[j] = RES2[[1,j]][2,1]
	
	TP_svm2[j] = RES2[[2,j]][2,2]
	TN_svm2[j] = RES2[[2,j]][1,1]
	FP_svm2[j] = RES2[[2,j]][1,2]
	FN_svm2[j] = RES2[[2,j]][2,1]
	
	TP_brglm2[j] = RES2[[3,j]][2,2]
	TN_brglm2[j] = RES2[[3,j]][1,1]
	FP_brglm2[j] = RES2[[3,j]][1,2]
	FN_brglm2[j] = RES2[[3,j]][2,1]
	
	TP_lda2[j] = RES2[[4,j]][2,2]
	TN_lda2[j] = RES2[[4,j]][1,1]
	FP_lda2[j] = RES2[[4,j]][1,2]
	FN_lda2[j] = RES2[[4,j]][2,1]
	
	TP_rf2[j] = RES2[[5,j]][2,2]
	TN_rf2[j] = RES2[[5,j]][1,1]
	FP_rf2[j] = RES2[[5,j]][1,2]
	FN_rf2[j] = RES2[[5,j]][2,1]
	
	TP_kknn2[j] = RES2[[6,j]][2,2]
	TN_kknn2[j] = RES2[[6,j]][1,1]
	FP_kknn2[j] = RES2[[6,j]][1,2]
	FN_kknn2[j] = RES2[[6,j]][2,1]
	
	TP_nn2[j] = RES2[[7,j]][2,2]
	TN_nn2[j] = RES2[[7,j]][1,1]
	FP_nn2[j] = RES2[[7,j]][1,2]
	FN_nn2[j] = RES2[[7,j]][2,1]
	
	TP_nb2[j] = RES2[[8,j]][2,2]
	TN_nb2[j] = RES2[[8,j]][1,1]
	FP_nb2[j] = RES2[[8,j]][1,2]
	FN_nb2[j] = RES2[[8,j]][2,1]
...
for (i in 1:(length(Names)))
{
	assign(paste("Total_TP_",Names[i],sep=""),sum(get(paste("TP_",Names[i],"2",sep=""))))
	assign(paste("Total_TN_",Names[i],sep=""),sum(get(paste("TN_",Names[i],"2",sep=""))))
	assign(paste("Total_FP_",Names[i],sep=""),sum(get(paste("FP_",Names[i],"2",sep=""))))
	assign(paste("Total_FN_",Names[i],sep=""),sum(get(paste("FN_",Names[i],"2",sep=""))))
